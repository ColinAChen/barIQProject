...
def evalPath(ePath='../data/samples', savePath='../output'):
    for path in os.listdir(ePath):
        ...
        #image = cv2.imread(imagePath)
        #green = segmentGreen(image)
        color = evalImage(imagePath)
        saveName = path.split('.')[0] + '.png'
...
def evalImage(path):
    ...
    bcDict, color = barcodeSearch(path)
    # save to 
